...
def convert_to_chat_format(text: str) -> List[Dict[str, str]]:
    """
    Convert a raw multi-turn QA or prompt-based dataset (format A)
    into a structured chat format (format B) with 'role' and 'content'.
    Assumes the entire text is one user query, and expects a paired assistant response externally.
    """
    return [
        {
            "content": text,
...
# Example usage
raw_text = """What are the most effective methods and strategies for accurately measuring the emotional intelligence and empathy of customer support agents? Can technology, such as sentiment analysis or facial recognition, be used to enhance the assessment process and provide deeper insights into agent performance? How can these methods be implemented in a Mark down code format?"""

converted = convert_to_chat_format(raw_text)
print(converted)
...
ds = load_dataset(ds_dir, split="train_prefs")
...
# Step 1: Convert prompt to chat format
ds = ds.map(
    lambda x: {
        "prompt_chat": convert_to_chat_format(x[script_args.dataset_key])
    }
)

# Step 2: Apply tokenizer to chat-formatted prompt
ds = ds.map(
    lambda x: {
        "prompt": tokenizer.apply_chat_template(
            x["prompt_chat"], tokenize=False, add_generation_prompt=True
        )
    }
)
